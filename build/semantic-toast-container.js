...
SemanticToastContainer = function (_Component) {_inherits(SemanticToastContainer, _Component);function SemanticToastContainer() {var _ref;var _temp, _this, _ret;_classCallCheck(this, SemanticToastContainer);for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {args[_key] = arguments[_key];}return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = SemanticToastContainer.__proto__ || Object.getPrototypeOf(SemanticToastContainer)).call.apply(_ref, [this].concat(args))), _this), _this.
            ...
            return (
                _react2.default.createElement('div', { className: 'ui-alerts ' + position + ' ' + className },
                    this.state.toasts.map(function (toast) {var
                        ...
                        toast.id,_toast$type = toast.type,type = _toast$type === undefined ? 'info' : _toast$type,_toast$title = toast.title,title = _toast$title === undefined ? '' : _toast$title,_toast$description = toast.description,description = _toast$description === undefined ? '' : _toast$description,_toast$icon = toast.icon,icon = _toast$icon === undefined ? 'announcement' : _toast$icon,time = toast.time;
                        return (
        ...
        } }]);return SemanticToastContainer;}(_react.Component);SemanticToastContainer.propTypes = { position: _propTypes2.default.oneOf(['top-right', 'top-center', 'bottom-right', 'bottom-center', 'bottom-left']), animation: _propTypes2.default.string };SemanticToastContainer.defaultProps = { position: 'top-right', animation: null };exports.default =
